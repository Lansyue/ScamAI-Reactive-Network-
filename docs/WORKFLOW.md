...
3. The Reactive contract checks whether the delay window has passed.
4. Once the settlement condition is met, the Reactive contract automatically calls the Destination contract.
5. The Destination contract pays the winner or keeps the prize pool intact when the challenge fails.
...
7. The Reactive contract calls `Destination.handleReactiveCallback(...)`.
8. If the player succeeded, the fixed reward is transferred on Polygon.
9. The frontend shows the final result together with the transaction trace.
